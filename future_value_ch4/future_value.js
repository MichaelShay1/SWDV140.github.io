...
document.addEventListener("DOMContentLoaded", () => {
    $("#calculate").addEventListener("click", () => {
        ...
        if (isNaN(investment) || investment <= 0 || investment > 100000) {
            errorMessage = "The Investment must be a number greater than 0 and less than or equal to 100,000";
            $("#investment").focus();
        } else if (isNaN(rate) || rate <= 0 || rate > 15) {
            errorMessage = "The Rate must be a number greater than 0 and less than or equal to 15";
            $("#rate").focus();
        } else if (isNaN(years) || years <= 0 || years > 50) {
            errorMessage = "The number of years must be a number greater than 0 and less than or equal to 50";
            $("#years").focus();
        }
        if (errorMessage == "") {
            $("#future_value").value = calculateFV(investment, rate, years);        
